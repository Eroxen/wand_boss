...
execute if data storage wandboss:calc math.ray2{hit:0b} run data modify storage wandboss:calc projectile set value {Tags:["wandboss.projectile.vampire","wandboss.projectile.tag_owner"],speed:10,duration:14}
...
execute if data storage wandboss:calc math.ray2{hit:1b} at @e[type=marker,tag=wandboss.math.raycast] run function wandboss:entity/projectile/vampire/hit
